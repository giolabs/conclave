...
- **Multi-sprint planning (v0.16.0+).** `/conclave-planning --all` plans every draft sprint from the backlog in one pass. The first sprint becomes `active`; the rest remain `draft` with their planning records written, ready to be activated when the prior sprint closes.
