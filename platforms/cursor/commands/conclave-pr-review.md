...
1. Parse `US-NNN`/`BUG-NNN`. If missing, ask via `AskQuestion` to pick from the list of `verified` stories in the active sprint (bugs are not offered in this picker — same rationale as `/conclave-dev`).
