...
8. After all batches complete, print the final summary table:
   ...
   ```
9. Stop. The individual steps below were already executed inside each Task call.
...
Encapsulate `/conclave-qa` for this story with **no `AskQuestion`**, model `MODEL_FOR_QA`, ledger += `qa`:
...
Use measured usage for a ledger row when the runtime exposes it for that Task call; otherwise estimate. Any mix of measured and estimated rows → `tokens_precision: mixed`.
...
2. Compute the **agent productivity** table, one row per role that was dispatched (`developer`, `designer`, `devops`, `qa`, `tech_lead`):
   - **Dispatches** — Task calls for that role in this run.
...
Three versioned templates under `skills/conclave/templates/`, posted to the Incoming Webhook whose URL is read from the env var **named** by `notifications.slack.webhook_env`:
