...
Copy `skills/conclave/board-app/` verbatim into `$REPO_ROOT/conclave-board/`. This is a plain file copy — no templating happens on these files; all per-team customization happens through `conclave/team/board.md`, read live by the app itself.

## Step 4 — Render `conclave/team/board.md`

If `$REPO_ROOT/conclave/team/board.md` does not already exist, render `skills/conclave/templates/board.template.md` (substituting `{{iso_date}}`) to `$REPO_ROOT/conclave/team/board.md`. If it already exists (e.g. a previous `/conclave-board` run was interrupted after this step), leave it untouched — never overwrite a team's branding edits.
