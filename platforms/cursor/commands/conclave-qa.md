...
2. `git switch $BRANCH` (or `git checkout $BRANCH`). If the branch does not exist locally, ask the user via `AskQuestion` whether to fetch it (`git fetch origin $BRANCH:$BRANCH`) or abort.
