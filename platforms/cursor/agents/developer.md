...
9. **Render the PR body** using `skills/conclave/templates/pr-body.template.md`. Fill the scenario→test mapping table and the DoD checklist. Return this PR body text to the orchestrator (it will use it for `gh pr create`).
...
1. **Never call `AskQuestion`.** There is no user to ask. If you were about to, choose one of two paths:
...
Interactive mode (the default, when the task prompt does not say `Autonomous mode`) is unaffected by this section — you may still use `AskQuestion` and the orchestrator will surface the prompt to the user.
