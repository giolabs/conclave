...
Ask the orchestrator to surface a clarifying question to the human Tech Lead via `AskQuestion`. Do not invent technical decisions the team would not stand behind.
...
- **Output**: one complete ADR markdown document matching `skills/conclave/templates/adr.template.md`. Fill every section — no `{{placeholder}}` strings left in prose. The orchestrator writes it to disk verbatim.
- **Hard rules**:
