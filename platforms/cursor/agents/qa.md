...
1. **Never call `AskQuestion` / `AskQuestion`.** The item ID, branch fetch, and CI-job decisions are already resolved by the orchestrator (or defaulted: fetch branch yes; decline new workflow writes).
