...
allowed-tools: Bash(git rev-parse:*), Bash(ls:*), Bash(find:*), Bash(date:*), Bash(mkdir:*), Bash(cat:*), Read, Write, Edit, Agent, AskUserQuestion
---
...
Issue **two `Agent` tool calls in a single message** so they run concurrently:
...
- Render `${CLAUDE_PLUGIN_ROOT}/skills/conclave/templates/sprint-meta.template.md` → `conclave/sprints/$SPRINT_ID/meta.md`.
- Render `${CLAUDE_PLUGIN_ROOT}/skills/conclave/templates/sprint-spec.template.md` → `conclave/sprints/$SPRINT_ID/spec.md`, with the selected stories table populated.
