...
- [Getting started](/getting-started) — install the plugin and run the Day-0 flow in a few minutes.
- [Team example](/team-example) — a worked example of several developers, each running Claude Code locally, delivering a sprint together.
- [Methodology](/methodology) — the Scrum model Conclave assumes and the accommodations it makes for real engineering teams.
- [Roles](/roles) — the seven role charters and which commands invoke each.
- [Team profiles](/profiles) — how to scale ceremony overhead to your team's size.
- [Configuration reference](/configuration) — every field in `conclave/config.md` explained.
- [Story state machine](/state-machine) — how a story moves from `backlog` to `done`, profile-aware.
- [Commands](/commands/init) — full reference for each of the six shipped slash commands.
