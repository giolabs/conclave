...
6. **Team profile** (`team` mode only — `solo` already forces `lean`): `lean`, `full-scrum`, or `custom` — controls which ceremonies are required. See [profiles](/en/profiles) for details.
