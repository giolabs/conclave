...
| `stack.*` | free text | Confirmed by `/conclave-spec`, not asked during `/conclave-init`. `stack.language` is the programming language, not the natural language used for docs. |
...
- **`commands.dev.interactive`** — `true` (default) or `false` (autonomous `/conclave-dev`, ending at `review`). See [`/conclave-dev`](/en/commands/dev).
...
- **`commands.dev.schedule`** — recurring local-time gate (see [Scheduling](/en/scheduling)). Conclave does not start itself — you supply `/loop`, an Automation, or `cron`. The pre-0.15.0 `window_start` / `window_end` pair is no longer honored: the loop prints a migration message and stops.
...
- **`commands.sprint.interactive`** — `true` (default) or `false` (headless one-pass with documented planning defaults). Since 0.15.0 it is not a delivery loop: no self-heal, no schedule, no budgets, no merge. `commands.sprint.schedule` / `budgets` / `merge_method` are ignored no-ops. See [`/conclave-sprint`](/en/commands/sprint).
