...
### Command configuration block (optional, v0.9.0+ / v0.13.0+)

Assign per-command interaction behavior.
...
#   sprint:
#     interactive: false          # Autonomous Sprint Loop (v0.13.0+) — may merge after QA+TL
#     merge_method: squash
#     schedule:
...
#       max_wall_clock_hours: 12       # exact weekend backstop
# notifications:
#   slack:
#     enabled: false
...
- **`commands.dev.interactive`** — `true` (default) or `false` (autonomous `/conclave-dev`). See [`/conclave-dev`](/en/commands/dev).
- **`commands.sprint.interactive`** (v0.13.0+) — `true` (default one-pass, **never merges**) or `false` (**Autonomous Sprint Loop**: serial self-heal → forced TL → merge → run report). CLI: `/conclave-sprint --no-interaction`. See [`/conclave-sprint`](/en/commands/sprint) and [Scheduling](/en/scheduling).
- **`commands.sprint.budgets.max_total_tokens`** — best-effort dispatch ledger with disclosed precision (`estimated` / `measured` / `mixed`). Set provider-side limits for a hard spend cap. Budgets are **per run**.
- **`commands.sprint.schedule`** — optional window gate; Conclave does not start itself — you supply `/loop`, Automation, or `cron`.
...
**Ad-hoc CLI override** — `/conclave-dev --no-interaction US-042` or `/conclave-sprint --no-interaction` (also `--headless`). There is no `--force-interaction` flag.
