...
When `peer_pr_review.required: true`, the **Tech Lead** (or designated approver) is the one who runs `gh pr review --approve` via `/conclave-pr-review US-NNN`. QA's job ends at the verification report and a PR comment — never an approval. See the [state machine](/en/state-machine) for how the story transitions.
